describe("Test Operators", function() {
  ...
  it("test equal/not equal", function() {
    ...
    var t = new jSmart("{if 5 eq 5}yes1{/if} {if 5 eq '5'}yes2{/if} {if true eq '1'}yes3{/if} {if true eq 'true'}yo{/if} {if 'bleh' eq 'bleh'}yup{/if}  {if 0 eq '1'}no{/if} {if 'ble' eq 'bleh'}naah{/if}");
    expect(t.fetch()).toBe('yes1 yes2 yes3  yup   ');

    var t = new jSmart("{if 4 != 4}no{/if} {if 4 != '4'}no{/if} {if '4' != 4}no{/if} {if '4' != '4'}no{/if} {if '3' != '4'}yes1{/if} {if true != false}yes2{/if} {if 'umakant' != 'Umakant'}yes3{/if}");
    expect(t.fetch()).toBe('    yes1 yes2 yes3');

    var t = new jSmart("{if 4 neq 4}no{/if} {if 4 neq '4'}no{/if} {if '4' neq 4}no{/if} {if '4' neq '4'}no{/if} {if '3' neq '4'}yes1{/if} {if true neq false}yes2{/if} {if 'umakant' neq 'Umakant'}yes3{/if}");
    expect(t.fetch()).toBe('    yes1 yes2 yes3');
  });
  ...
  it("test operators priority", function() {
    ...
    var t = new jSmart("{if $a is not div by 4}test1{/if} {if $a is div by 5}test2{/if}");
    expect(t.fetch({a: 15, b:10})).toBe('test1 test2');

    var t = new jSmart("{if $c is not even}test-1{/if} {if $b is even}test0{/if} {if $a is not even by $b}test1{/if} {if $a is even by $c}test2{/if} {if ($a/$b) % 2  != 0}test3{/if} {if ($a/$c)%2 == 0}test4{/if}");
    expect(t.fetch({a: 6, b:2, c:3})).toBe('test-1 test0 test1 test2 test3 test4');

    var t = new jSmart("{if $b is not odd}test-1{/if} {if $c is odd}test0{/if} {if $a is not odd by $c}test1{/if} {if $a is odd by $b}test2{/if} {if ($a/$b) % 2  != 0}test3{/if} {if ($a/$c)%2 == 0}test4{/if}");
    expect(t.fetch({a: 6, b:2, c:3})).toBe('test-1 test0 test1 test2 test3 test4');
  });
  ...
  it("test and/or/xor", function() {
    ...
    var t = new jSmart("{if true || true}test1{/if} {if true or false}test2{/if} {if 1 or true}test3{/if} {if 'test' or true}test4{/if} {if 'test' ||  'test'}test5{/if} {if $a || $b}test6{/if} {if $a || $c}test7{/if}");
    expect(t.fetch({a: false, b: false, c: 'yo'})).toBe('test1 test2 test3 test4 test5  test7');

    var t = new jSmart("{if false xor false}test1{/if} {if true xor false}test2{/if} {if 1 xor true}test3{/if} {if 'test' xor true}test4{/if} {if $a xor $b}test5{/if} {if $a xor $c}test6{/if} {if $b xor $d}test7{/if}");
    expect(t.fetch({a: false, b: true, c:false, d: true})).toBe(' test2   test5  ');

    var t = new jSmart("{if ($a || $b) && !($a && $b)}test1{/if} {if ($a || $c) && !($a && $c)}test2{/if}");
